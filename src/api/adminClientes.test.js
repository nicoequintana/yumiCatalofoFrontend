...
describe("getResumenClientes — período por dias", () => {
  ...
  it("manda dias como query param", async () => {
    ...
    await getResumenClientes({ dias: 30 });

    expect(fetchAutenticado).toHaveBeenCalledWith(`${BASE}/admin/clientes-resumen?dias=30`, undefined);
  });
});
