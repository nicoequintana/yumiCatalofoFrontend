...
function AdminUsuarios() {
   ...
   * "Todavía no hay usuarios" debajo de un error de red le afirma al admin que
   * la única cuenta del panel es la suya. Mismo criterio que
   * `AdminCategorias` y `AdminProductos`.
   */
  ...
  useEffect(() => {
    ...
    getUsuarios()
      .then((data) => {
        if (!activo) return;
        setUsuarios(data);
        // Un fetch exitoso limpia el error anterior.
        setErrorCarga(false);
        setCargando(false);
      })
  ...
  return (
    <main className="w-full px-4 py-6 md:px-8 md:py-8">
      ...
      <form onSubmit={handleCrear} className="mb-8 flex flex-col gap-3 sm:flex-row">
        ...
        {/* El área táctil la lleva el LABEL, no la casilla. Un
            `<input type="checkbox">` es un elemento reemplazado y no admite
            `::before`, así que el pseudo-elemento de `areaTactil.js` no le
            sirve; agrandar la casilla con `size-11`, en cambio, la dibujaría
            como un cuadro enorme. Como el label envuelve al input, un click en
            cualquier punto suyo marca la casilla: darle `min-h-11` convierte
            toda la franja de 44 de alto en área táctil real. Medido el
            07/09/2026 con `elementFromPoint`: 16x17 en los dos anchos, el peor
            control de la auditoría. */}
        <label className="font-body-md text-body-md flex min-h-11 items-center gap-2 text-on-surface">
          <input
            type="checkbox"
            ...
            <tbody role="rowgroup">
              {usuarios.map((usuario) => (
                <tr key={usuario.id} role="row" className="border-b border-outline-variant last:border-b-0">
                  ...
                  >
                    {editandoId === usuario.id ? (
                      // Mismo criterio que el checkbox del alta: el área de
                      // 44 la lleva el label que envuelve a la casilla. No lo
                      // contó la auditoría del 07/09/2026 porque sólo se
                      // renderiza en modo edición, pero es idéntico.
